...
def gridshow(name, imgs, scales, cmaps, width, border=10):
    ...
    for i, (img, cmap, scale) in enumerate(zip(imgs, cmaps, scales)):
        ...
        if scale is not None:
            img = (np.clip(img, scale[0], scale[1]) - scale[0]) / (scale[1] -
                                                                   scale[0])
        ...
        if i > 0 and i % width == (width - 1):
            imgrows.append(
                np.hstack([
                    np.pad(c, ((0, maxh - c.shape[0]),
                               (border // 2, border // 2), (0, 0)),
                           mode='constant') for c in imgcols
                ]))
            imgcols = []
            maxh = 0
    ...
    if imgcols:
        imgrows.append(
            np.hstack([
                np.pad(c, ((0, maxh - c.shape[0]), (border // 2, border // 2),
                           (0, 0)),
                       mode='constant') for c in imgcols
            ]))
    ...
    cv2.imshow(
        name,
        np.vstack([
            np.pad(r, ((border // 2, border // 2), (0, maxw - r.shape[1]),
                       (0, 0)),
