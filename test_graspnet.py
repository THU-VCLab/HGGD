...
from dataset.grasp import GraspGroup as OurGraspGroup
...
from ourgraspnetAPI import Grasp as GraspNetGrasp
from ourgraspnetAPI import GraspGroup as GraspNetGraspGroup
from ourgraspnetAPI import GraspNetEval
...
parser.add_argument('--loc-a', type=float, default=1)
parser.add_argument('--reg-b', type=float, default=5)
parser.add_argument('--cls-c', type=float, default=1)
...
def grasp_filter(camera_pose, gg):
    rotations = gg.rotations.copy()
    translations = gg.translations.copy()
    # transform to matrix
    T_grasp_in_camera = np.zeros((len(translations), 4, 4))
    T_grasp_in_camera[:, :3, :3] = rotations
    T_grasp_in_camera[:, :3, 3] = translations
    T_grasp_in_camera[:, 3, 3] = 1
    # get camera pose
    T_camera_in_base = camera_pose
    T_grasp_in_base = np.einsum('ij,njk->nik', T_camera_in_base,
                                T_grasp_in_camera)
    # filter
    translations_base = T_grasp_in_base[:, :3, 3]
    print(translations_base.max(0), translations_base.min(0))
    rotations_base = T_grasp_in_base[:, :3, :3]
    mask = np.logical_and(translations_base[:, 2] > 0.01,
                          translations_base[:, 2] < 1)
    # make grasp is along to camera orientation and not that parallel to desk
    mask_parallel = np.logical_and(rotations_base[:, 2, 0] < 0,
                                   rotations[:, 2, 0] > 0)
    mask = np.logical_and(mask, mask_parallel)
    print('grasp before filter ==', len(mask))
    print('grasp after filter ==', mask.sum())
    gg = gg[mask]
    return gg
...
def inference():
    ...
    batch_idx = -1
    # bad scene: [5, 7, 11, 13, 14, 20, 24, 25]
    vis_id = []
    # vis_id = [_ for _ in range(0, 256)]
    with torch.no_grad():
        for anchor_data, rgb, ori_depth, grasppaths in test_data:
            ...
            valid_num = args.center_num - zero_pad_num
            ...
            for idx_g, pred_g in enumerate(pred_gg):
            ...
            pred_gg = OurGraspGroup(translations=gg.translations,
