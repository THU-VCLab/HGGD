...
from .utils import (generate_scene_model, generate_scene_pointcloud,
                    generate_views, get_model_grasps, plot_gripper_pro_max,
                    transform_points)
...
def visAnno(dataset_root,
    ...
    **Input:**
    ...
    model_list, obj_list, pose_list = generate_scene_model(
        ...
        anno_idx,
        return_poses=True,
        align=align_to_table,
        camera=camera)
    point_cloud = generate_scene_pointcloud(dataset_root,
                                            scene_name,
                                            anno_idx,
                                            align=align_to_table,
                                            camera=camera)

    table = create_table_cloud(1.0,
                               0.02,
                               1.0,
                               dx=-0.5,
                               dy=-0.5,
    ...
    collision_label = np.load(
        '{}/collision_label/{}/collision_labels.npz'.format(
    ...
    for i, (obj_idx, trans) in enumerate(zip(obj_list, pose_list)):
            ...
            for v in view_inds:
                ...
                for a in angle_inds:
                    ...
                    for d in depth_inds:
                        ...
                        t = transform_points(target_point[np.newaxis, :],
                                             trans).squeeze()
                        R = np.dot(trans[:3, :3], R)
                        gripper = plot_gripper_pro_max(t, R, width, depth,
                                                       1.1 - score[v, a, d])
        ...
        if per_obj:
            ...
            filename = os.path.join(
                save_folder,
                '{}_{}_pointcloud_{}.png'.format(scene_name, camera, obj_idx))
    ...
    if not per_obj:
        ...
        filename = os.path.join(save_folder,
                                '{}_{}_model.png'.format(scene_name, camera))
        vis.capture_screen_image(filename, do_render=True)
        if show:
            o3d.visualization.draw_geometries([table, *model_list, *grippers])


def vis6D(dataset_root,
          scene_name,
          anno_idx,
          camera,
          align_to_table=True,
    ...
    **Input:**
    ...
    if per_obj:
        for i, model in zip(obj_list, model_list):
            ...
            vis.poll_events()
            filename = os.path.join(
                save_folder, '{}_{}_6d_{}.png'.format(scene_name, camera, i))
            vis.capture_screen_image(filename, do_render=True)
        ...
        vis.poll_events()
        filename = os.path.join(save_folder,
                                '{}_{}_6d.png'.format(scene_name, camera))
        vis.capture_screen_image(filename, do_render=True)
        if show:
            o3d.visualization.draw_geometries([point_cloud, *model_list])


def visObjGrasp(dataset_root,
                obj_idx,
                num_grasp=10,
                th=0.5,
                max_width=0.08,
    ...
    **Input:**
    ...
    cam_pos = np.load(
        os.path.join(dataset_root, 'scenes', 'scene_0000', 'kinect',
                     'cam0_wrt_table.npy'))
    param.extrinsic = np.linalg.inv(cam_pos).tolist()
...
def vis_rec_grasp(rec_grasp_tuples,
                  numGrasp,
                  image_path,
                  save_path,
                  show=False):
