...
class xmlWriter():
    ...
    def objectlistfromposevectorlist(self, posevectorlist, objdir, objnamelist,
                                     objidlist):
        self.poselist = []
        self.objnamelist = []
        self.objidlist = []
        ...
        for i in range(len(posevectorlist)):
            ...
            self.addobject(get_mat(x, y, z, alpha, beta, gamma), objname,
                           os.path.join(objdir, objname), id)
...
def getposevectorlist(objectidlist, is_resume, num_frame, frame_number,
                      xml_dir):
    if not is_resume or (not os.path.exists(
...
def getframeposevectorlist(objectidlist, is_resume, frame_number, xml_dir):
    ...
    for num_frame in range(frame_number):
        if not is_resume or (not os.path.exists(
                os.path.join(xml_dir, '%04d.xml' % num_frame))):
            posevectorlist = getposevectorlist(objectidlist, False, num_frame,
                                               frame_number, xml_dir)
        else:
            posevectorlist = getposevectorlist(objectidlist, True, num_frame,
                                               frame_number, xml_dir)
