...
from .utils.utils import (batch_center_depth, batch_framexy_depth_2_xyz,
                          batch_key_point_2_rotation, batch_key_points_2_tuple,
                          batch_rgbdxyz_2_rgbxy_depth, center_depth,
                          framexy_depth_2_xyz, get_batch_key_points,
                          key_point_2_rotation, plot_gripper_pro_max)
...
class Grasp():
        ...
        **Input:**
        ...
        if len(args) == 0:
            self.grasp_array = np.array(
                [0, 0.02, 0.02, 0.02, 1, 0, 0, 0, 1, 0, 0, 0, 1, 0, 0, 0, -1],
                dtype=np.float64)
        elif len(args) == 1:
            if type(args[0]) == np.ndarray:
                ...
                raise TypeError(
                    'if only one arg is given, it must be np.ndarray.')
        elif len(args) == 7:
            score, width, height, depth, rotation_matrix, translation, object_id = args
            self.grasp_array = np.concatenate([
                np.array((score, width, height, depth)),
                rotation_matrix.reshape(-1), translation,
                np.array((object_id)).reshape(-1)
            ]).astype(np.float64)
        else:
    ...
    def __repr__(self):
        return 'Grasp: score:{}, width:{}, height:{}, depth:{}, translation:{}\nrotation:\n{}\nobject id:{}'.format(
            self.score, self.width, self.height, self.depth, self.translation,
            self.rotation_matrix, self.object_id)
    ...
    def rotation_matrix(self, *args):
        ...
        **Input:**
        ...
        '''
        if len(args) == 1:
            self.grasp_array[4:13] = np.array(args[0],
    ...
    def transform(self, T):
        ...
        **Output:**
        ...
        rotation = T[:3, :3]
        translation = T[:3, 3]
        self.translation = np.dot(rotation, self.translation.reshape(
            (3, 1))).reshape(-1) + translation
    ...
    def to_open3d_geometry(self, color=None):
        ...
        **Ouput:**
        ...
        '''
        return plot_gripper_pro_max(self.translation,
...
class GraspGroup():
    ...
    def __init__(self, *args):
        ...
        **Input:**
        ...
        '''
        if len(args) == 0:
            self.grasp_group_array = np.zeros((0, GRASP_ARRAY_LEN),
                                              dtype=np.float64)
        elif len(args) == 1:
            if isinstance(args[0], np.ndarray):
                ...
                raise ValueError(
                    'args must be nothing, numpy array or string.')
        else:
    ...
    def __repr__(self):
        ...
        if self.__len__() <= 6:
            ...
            for i in range(3):
                repr += Grasp(
                    self.grasp_group_array[-(3 - i)]).__repr__() + '\n'
        return repr + '----------'
    ...
    def __getitem__(self, index):
        ...
        **Output:**
        ...
        if type(index) == int:
            ...
            graspgroup.grasp_group_array = copy.deepcopy(
                self.grasp_group_array[index])
            return graspgroup
    ...
    def transform(self, T):
        ...
        **Output:**
        ...
        rotation = T[:3, :3]
        translation = T[:3, 3]
        self.translations = np.dot(
            rotation, self.translations.T).T + translation  # (-1, 3)
        self.rotation_matrices = np.matmul(rotation,
                                           self.rotation_matrices).reshape(
                                               (-1, 3, 3))  # (-1, 9)
        return self
    ...
    def remove(self, index):
        ...
        **Input:**
        ...
        '''
        self.grasp_group_array = np.delete(self.grasp_group_array,
                                           index,
                                           axis=0)
        return self
    ...
    def nms(self,
        ...
        **Output:**
        ...
        return GraspGroup(
            nms_grasp(self.grasp_group_array, translation_thresh,
                      rotation_thresh))
...
class RectGrasp():
    ...
    def __repr__(self):
        return 'Rectangle Grasp: score:{}, height:{}, open point:{}, center point:{}, object id:{}'.format(
            self.score, self.height, self.open_point, self.center_point,
            self.object_id)
    ...
    def to_opencv_image(self, opencv_rgb):
        ...
        **Output:**
        ...
        cv2.line(opencv_rgb, (int(p1[0]), int(p1[1])),
                 (int(p2[0]), int(p2[1])), (0, 0, 255), 1, 8)
        cv2.line(opencv_rgb, (int(p2[0]), int(p2[1])),
                 (int(p3[0]), int(p3[1])), (255, 0, 0), 3, 8)
        cv2.line(opencv_rgb, (int(p3[0]), int(p3[1])),
                 (int(p4[0]), int(p4[1])), (0, 0, 255), 1, 8)
        cv2.line(opencv_rgb, (int(p4[0]), int(p4[1])),
                 (int(p1[0]), int(p1[1])), (255, 0, 0), 3, 8)
        return opencv_rgb
    ...
    def get_key_points(self):
        ...
        **Output:**
        ...
        unit_open_point_vector = open_point_vector / np.linalg.norm(
            open_point_vector)
        counter_clock_wise_rotation_matrix = np.array([[0, -1], [1, 0]])
        upper_point = np.dot(counter_clock_wise_rotation_matrix,
                             unit_open_point_vector) * height / 2 + center
    ...
    def to_grasp(self, camera, depths, depth_method=center_depth):
        ...
        **Output:**
        ...
        depth_2d = depth_method(depths, center, open_point,
                                upper_point) / 1000.0
        # print('depth 2d:{}'.format(depth_2d))
        if abs(depth_2d) < EPS:
            return None
        center_xyz = np.array(
            framexy_depth_2_xyz(center[0], center[1], depth_2d, camera))
        open_point_xyz = np.array(
            framexy_depth_2_xyz(open_point[0], open_point[1], depth_2d,
                                camera))
        upper_point_xyz = np.array(
            framexy_depth_2_xyz(upper_point[0], upper_point[1], depth_2d,
                                camera))
        depth = 0.02
        ...
        rotation = key_point_2_rotation(center_xyz, open_point_xyz,
                                        upper_point_xyz)
        # to avoid bug some time
        if height < EPS:
            return None
        return Grasp(score, width, height, depth, rotation, translation,
                     object_id)
...
class RectGraspGroup():
    ...
    def __init__(self, *args):
        ...
        **Input:**
        ...
        '''
        if len(args) == 0:
            self.rect_grasp_group_array = np.zeros((0, RECT_GRASP_ARRAY_LEN),
                                                   dtype=np.float64)
        elif len(args) == 1:
            if isinstance(args[0], np.ndarray):
    ...
    def __repr__(self):
        ...
        if self.__len__() <= 10:
            ...
            for i in range(5):
                repr += RectGrasp(
                    self.rect_grasp_group_array[i]).__repr__() + '\n'
            repr += '......\n'
            for i in range(5):
                repr += RectGrasp(
                    self.rect_grasp_group_array[-(5 - i)]).__repr__() + '\n'
        return repr + '----------'
    ...
    def __getitem__(self, index):
        ...
        **Output:**
        ...
        elif isinstance(index, slice) or isinstance(index, list) or isinstance(
                index, np.ndarray):
            rectgraspgroup = RectGraspGroup()
            rectgraspgroup.rect_grasp_group_array = copy.deepcopy(
                self.rect_grasp_group_array[index])
    ...
    def add(self, rect_grasp):
        ...
        **Input:**
        ...
        '''
        self.rect_grasp_group_array = np.concatenate(
            (self.rect_grasp_group_array,
             rect_grasp.rect_grasp_array.reshape((-1, RECT_GRASP_ARRAY_LEN))))
        return self
    ...
    def remove(self, index):
        ...
        **Input:**
        ...
        '''
        self.rect_grasp_group_array = np.delete(self.rect_grasp_group_array,
                                                index,
                                                axis=0)
    ...
    def batch_get_key_points(self):
        ...
        **Output:**
        ...
        upper_points = np.einsum(
            ...
            unit_open_point_vector.reshape(-1, 2, 1)).reshape(
                -1, 2) * np.hstack([heights, heights]) / 2 + centers  # (-1, 2)
    ...
    def to_grasp_group(self, camera, depths, depth_method=batch_center_depth):
        ...
        **Output:**
        ...
        depths_2d = depth_method(depths, centers, open_points,
                                 upper_points) / 1000.0
        ...
        rotations = batch_key_point_2_rotation(centers_xyz, open_points_xyz,
                                               upper_points_xyz).reshape(
                                                   (-1, 9))
        grasp_group = GraspGroup()
        grasp_group.grasp_group_array = copy.deepcopy(
            np.hstack((scores, widths, heights, depths, rotations,
                       translations, object_ids))).astype(np.float64)
        return grasp_group
    ...
    def random_sample(self, numGrasp=20):
        ...
        **Output:**
        ...
        shuffled_rect_grasp_group.rect_grasp_group_array = copy.deepcopy(
            shuffled_rect_grasp_group_array[:numGrasp])
