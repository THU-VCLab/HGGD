...
from .grasp import (RECT_GRASP_ARRAY_LEN, Grasp, GraspGroup, RectGrasp,
                    RectGraspGroup)
from .utils.utils import parse_posevector, transform_points
...
class GraspNet():
    ...
    def __init__(self, root, camera='kinect', split='train'):
        ...
        **input**:
        ...
        """
        assert camera in ['kinect',
                          'realsense'], 'camera should be kinect or realsense'
        # assert split in [
        #     'all', 'train', 'test', 'test_seen', 'test_similar', 'test_novel'
        # ], 'split should be all/train/test/test_seen/test_similar/test_novel'
        self.root = root
        self.camera = camera
        self.split = split
        ...
        if split == 'all':
            ...
            self.sceneIds = list(range(split[0], split[1]))
        ...
        for i in tqdm(self.sceneIds, desc='Loading data path...'):
            for img_num in range(256):
                self.rgbPath.append(
                    os.path.join(root, 'scenes', 'scene_' + str(i).zfill(4),
                                 camera, 'rgb',
        ...
        **Output:**
        ...
        for obj_id in tqdm(range(88), 'Checking Models'):
            if not os.path.exists(
                    os.path.join(self.root, 'models', '%03d' % obj_id,
                                 'nontextured.ply')):
                error_flag = True
        ...
        for obj_id in tqdm(range(88), 'Checking Grasp Labels'):
            if not os.path.exists(
                    os.path.join(self.root, 'grasp_label',
                                 '%03d_labels.npz' % obj_id)):
                error_flag = True
                print('No Grasp Label For Object {}'.format(obj_id))
        for sceneId in tqdm(self.sceneIds, 'Checking Collosion Labels'):
            if not os.path.exists(
                    os.path.join(self.root, 'collision_label', 'scene_%04d' %
                                 sceneId, 'collision_labels.npz')):
                error_flag = True
        ...
        for sceneId in tqdm(self.sceneIds, 'Checking Scene Datas'):
            scene_dir = os.path.join(self.root, 'scenes',
                                     'scene_%04d' % sceneId)
            if not os.path.exists(os.path.join(scene_dir,
                                               'object_id_list.txt')):
                error_flag = True
            ...
            for camera in [self.camera]:
                ...
                if not os.path.exists(
                        os.path.join(camera_dir, 'cam0_wrt_table.npy')):
                    error_flag = True
                    print(
                        'No cam0_wrt_table.npy For Scene {}, Camera:{}'.format(
                            sceneId, camera))
                if not os.path.exists(
                        os.path.join(camera_dir, 'camera_poses.npy')):
                    error_flag = True
                    print('No camera_poses.npy For Scene {}, Camera:{}'.format(
                        sceneId, camera))
                if not os.path.exists(os.path.join(camera_dir, 'camK.npy')):
                    error_flag = True
                    print('No camK.npy For Scene {}, Camera:{}'.format(
                        sceneId, camera))
                for annId in range(256):
                    if not os.path.exists(
                            os.path.join(camera_dir, 'rgb',
                                         '%04d.png' % annId)):
                        error_flag = True
                        print(
                            'No RGB Image For Scene {}, Camera:{}, annotion:{}'
                            .format(sceneId, camera, annId))
                    if not os.path.exists(
                            os.path.join(camera_dir, 'depth',
                                         '%04d.png' % annId)):
                        error_flag = True
                        print(
                            'No Depth Image For Scene {}, Camera:{}, annotion:{}'
                            .format(sceneId, camera, annId))
                    if not os.path.exists(
                            os.path.join(camera_dir, 'label',
                                         '%04d.png' % annId)):
                        error_flag = True
                        print(
                            'No Mask Label image For Scene {}, Camera:{}, annotion:{}'
                            .format(sceneId, camera, annId))
                    if not os.path.exists(
                            os.path.join(camera_dir, 'meta',
                                         '%04d.mat' % annId)):
                        error_flag = True
                        print(
                            'No Meta Data For Scene {}, Camera:{}, annotion:{}'
                            .format(sceneId, camera, annId))
                    if not os.path.exists(
                            os.path.join(camera_dir, 'annotations',
                                         '%04d.xml' % annId)):
                        error_flag = True
                        print(
                            'No Annotations For Scene {}, Camera:{}, annotion:{}'
                            .format(sceneId, camera, annId))
                    if not os.path.exists(
                            os.path.join(camera_dir, 'rect',
                                         '%04d.npy' % annId)):
                        error_flag = True
                        print(
                            'No Rectangle Labels For Scene {}, Camera:{}, annotion:{}'
                            .format(sceneId, camera, annId))
    ...
    def getSceneIds(self, objIds=None):
        ...
        **Output:**
        ...
        assert _isArrayLike(objIds) or isinstance(
            objIds,
            int), 'objIds must be integer or a list/numpy array of integers'
        objIds = objIds if _isArrayLike(objIds) else [objIds]
        sceneIds = []
        for i in self.sceneIds:
            f = open(
                os.path.join(self.root, 'scenes', 'scene_' + str(i).zfill(4),
                             'object_id_list.txt'))
    ...
    def getObjIds(self, sceneIds=None):
        ...
        **Output:**
        ...
        if sceneIds is None:
            return self.objIds
        assert _isArrayLike(sceneIds) or isinstance(
            sceneIds, int
        ), 'sceneIds must be an integer or a list/numpy array of integers'
    ...
    def getDataIds(self, sceneIds=None):
        ...
        **Output:**
        ...
        for i in sceneIds:
            indexPosList += [
                j for j in range(0, len(self.sceneName), 256)
                if self.sceneName[j] == 'scene_' + str(i).zfill(4)
            ]
    ...
    def loadObjModels(self, objIds=None):
        ...
        **Output:**
        ...
        for i in objIds:
            ...
            plyfile = os.path.join(self.root, 'models', '%03d' % i,
                                   'nontextured.ply')
    ...
    def loadCollisionLabels(self, sceneIds=None):
        ...
        **Output:**
        ...
        for sid in sceneIds:
            ...
            labels = np.load(
                os.path.join(self.root, 'collision_label',
                             'scene_' + str(sid).zfill(4),
    ...
    def loadRGB(self, sceneId, camera, annId):
        ...
        **Output:**
        ...
        return cv2.cvtColor(
            cv2.imread(
                os.path.join(self.root, 'scenes', 'scene_%04d' % sceneId,
                             camera, 'rgb', '%04d.png' % annId)),
            cv2.COLOR_BGR2RGB)
    ...
    def loadBGR(self, sceneId, camera, annId):
        ...
        **Output:**
        ...
        '''
        return cv2.imread(
            os.path.join(self.root, 'scenes', 'scene_%04d' % sceneId, camera,
                         'rgb', '%04d.png' % annId))
    ...
    def loadDepth(self, sceneId, camera, annId):
        ...
        **Output:**
        ...
        return cv2.imread(
            os.path.join(self.root, 'scenes', 'scene_%04d' % sceneId, camera,
                         'depth', '%04d.png' % annId), cv2.IMREAD_UNCHANGED)
    ...
    def loadMask(self, sceneId, camera, annId):
        ...
        **Output:**
        ...
        return cv2.imread(
            os.path.join(self.root, 'scenes', 'scene_%04d' % sceneId, camera,
                         'label', '%04d.png' % annId), cv2.IMREAD_UNCHANGED)
    ...
    def loadScenePointCloud(self,
        ...
        **Output:**
        ...
        '''
        colors = self.loadRGB(sceneId=sceneId, camera=camera,
                              annId=annId).astype(np.float32) / 255.0
    ...
    def loadSceneModel(self, sceneId, camera='kinect', annId=0, align=False):
        ...
        **Output:**
        ...
        for obj_idx, pose in zip(obj_list, mat_list):
            plyfile = os.path.join(self.root, 'models', '%03d' % obj_idx,
                                   'nontextured.ply')
    ...
    def loadGrasp(self,
        ...
        **Output:**
        ...
        if format == '6d':
            ...
            from .utils.utils import (generate_views, get_model_grasps,
                                      get_obj_pose_list, transform_points)
            from .utils.xmlhandler import xmlReader
            ...
            scene_reader = xmlReader(
                os.path.join(self.root, 'scenes', 'scene_%04d' % (sceneId, ),
                             camera, 'annotations', '%04d.xml' % (annId, )))
            ...
            template_views = template_views[np.newaxis, :, np.newaxis,
                                            np.newaxis, :]
            template_views = np.tile(template_views,
                                     [1, 1, num_angles, num_depths, 1])
            ...
            for i, (obj_idx, trans) in enumerate(zip(obj_list, pose_list)):
                ...
                target_points = sampled_points[:, np.newaxis, np.newaxis,
                                               np.newaxis, :]
                target_points = np.tile(
                    target_points, [1, num_views, num_angles, num_depths, 1])
                ...
                target_points = transform_points(target_points,
                ...
                Rs = np.matmul(
                    np.linalg.inv(camera_pose)[np.newaxis, :3, :3], Rs)
                ...
                obj_grasp_array = np.hstack([
                    scores, widths, heights, depths, rotations, target_points,
                    object_ids
                ]).astype(np.float32)

                grasp_group.grasp_group_array = np.concatenate(
                    (grasp_group.grasp_group_array, obj_grasp_array))
            ...
            # 'rect'
            rect_grasps = RectGraspGroup(
                os.path.join(self.root, 'scenes', 'scene_%04d' % sceneId,
                             camera, 'rect', '%04d.npy' % annId))
            return rect_grasps
    ...
    def loadData(self, ids=None, *extargs):
        ...
        **Output:**
        ...
        '''
        if ids is None:
            return (self.rgbPath, self.depthPath, self.segLabelPath,
                    self.metaPath, self.rectLabelPath, self.sceneName,
                    self.annId)

        if len(extargs) == 0:
            if isinstance(ids, int):
                return (self.rgbPath[ids], self.depthPath[ids],
                        self.segLabelPath[ids], self.metaPath[ids],
                        self.rectLabelPath[ids], self.sceneName[ids],
                        self.annId[ids])
            else:
                return ([self.rgbPath[id]
                         for id in ids], [self.depthPath[id] for id in ids],
                        [self.segLabelPath[id]
                         for id in ids], [self.metaPath[id] for id in ids],
                        [self.rectLabelPath[id]
                         for id in ids], [self.sceneName[id] for id in ids
                                          ], [self.annId[id] for id in ids])
        if len(extargs) == 2:
            sceneId = ids
            camera, annId = extargs
            rgbPath = os.path.join(self.root, 'scenes',
                                   'scene_' + str(sceneId).zfill(4), camera,
                                   'rgb',
                                   str(annId).zfill(4) + '.png')
            depthPath = os.path.join(self.root, 'scenes',
                                     'scene_' + str(sceneId).zfill(4), camera,
                                     'depth',
                                     str(annId).zfill(4) + '.png')
            segLabelPath = os.path.join(self.root, 'scenes',
                                        'scene_' + str(sceneId).zfill(4),
                                        camera, 'label',
                                        str(annId).zfill(4) + '.png')
            metaPath = os.path.join(self.root, 'scenes',
                                    'scene_' + str(sceneId).zfill(4), camera,
                                    'meta',
                                    str(annId).zfill(4) + '.mat')
            rectLabelPath = os.path.join(self.root, 'scenes',
                                         'scene_' + str(sceneId).zfill(4),
                                         camera, 'rect',
                                         str(annId).zfill(4) + '.npy')
            scene_name = 'scene_' + str(sceneId).zfill(4)
            return (rgbPath, depthPath, segLabelPath, metaPath, rectLabelPath,
                    scene_name, annId)

    def showObjGrasp(self,
                     objIds=[],
                     numGrasp=10,
                     th=0.5,
                     maxWidth=0.08,
    ...
    def showSceneGrasp(self,
        ...
        **Input:**
        ...
        if format == '6d':
            geometries = []
            sceneGrasp = self.loadGrasp(sceneId=sceneId,
                                        annId=annId,
                                        camera=camera,
                                        format='6d',
                                        fric_coef_thresh=coef_fric_thresh)
            sceneGrasp = sceneGrasp.random_sample(numGrasp=numGrasp)
            scenePCD = self.loadScenePointCloud(sceneId=sceneId,
                                                camera=camera,
                                                annId=annId,
                                                align=False)
            ...
            if show_object:
                objectPCD = self.loadSceneModel(sceneId=sceneId,
                                                camera=camera,
                                                annId=annId,
                                                align=False)
