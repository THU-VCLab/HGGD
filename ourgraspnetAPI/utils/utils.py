...
def generate_scene_model(dataset_root,
                         scene_name,
                         anno_idx,
                         return_poses=False,
                         align=False,
    ...
    **Output:**
    ...
    if align:
        camera_poses = np.load(
            os.path.join(dataset_root, 'scenes', scene_name, camera,
                         'camera_poses.npy'))
    ...
    scene_reader = xmlReader(
        os.path.join(dataset_root, 'scenes', scene_name, camera, 'annotations',
                     '%04d.xml' % anno_idx))
    ...
    for obj_idx, pose in zip(obj_list, mat_list):
        plyfile = os.path.join(dataset_root, 'models', '%03d' % obj_idx,
                               'nontextured.ply')
...
def generate_scene_pointcloud(dataset_root,
                              scene_name,
                              anno_idx,
                              align=False,
                              camera='kinect'):
    ...
    **Output:**
    ...
    '''
    colors = np.array(Image.open(
        os.path.join(dataset_root, 'scenes', scene_name, camera, 'rgb',
                     '%04d.png' % anno_idx)),
                      dtype=np.float32) / 255.0
    depths = np.array(
        Image.open(
            os.path.join(dataset_root, 'scenes', scene_name, camera, 'depth',
                         '%04d.png' % anno_idx)))
    intrinsics = np.load(
        os.path.join(dataset_root, 'scenes', scene_name, camera, 'camK.npy'))
...
def rotation_matrix(rx, ry, rz):
    ...
    **Output:**
    ...
    Rz = np.array([[np.cos(rz), -np.sin(rz), 0], [np.sin(rz),
                                                  np.cos(rz), 0], [0, 0, 1]])
    R = Rz.dot(Ry).dot(Rx)
    return R
...
def viewpoint_params_to_matrix(towards, angle):
    ...
    **Output:**
    ...
    axis_y = axis_y / np.linalg.norm(axis_y)
    axis_z = np.cross(axis_x, axis_y)
    R1 = np.array([[1, 0, 0], [0, np.cos(angle), -np.sin(angle)],
...
def dexnet_params_to_matrix(binormal, angle):
    ...
    **Output:**
    ...
    axis_y = axis_y / np.linalg.norm(axis_y)
    axis_z = np.cross(axis_x, axis_y)
    R1 = np.array([[np.cos(angle), 0, np.sin(angle)], [0, 1, 0],
...
def create_mesh_box(width, height, depth, dx=0, dy=0, dz=0):
    ...
    vertices = np.array([[0, 0, 0], [width, 0, 0], [0, 0, depth],
                         [width, 0, depth], [0, height, 0], [width, height, 0],
                         [0, height, depth], [width, height, depth]])
    vertices[:, 0] += dx
    vertices[:, 1] += dy
    vertices[:, 2] += dz
    triangles = np.array([[4, 7, 5], [4, 6, 7], [0, 2, 4], [2, 6,
                                                            4], [0, 1, 2],
                          [1, 3, 2], [1, 5, 7], [1, 7, 3], [2, 3, 7],
                          [2, 7, 6], [0, 4, 1], [1, 4, 5]])
...
def plot_gripper_pro_max(center, R, width, depth, score=1, color=None):
    ...
    **Output:**
    ...
    left = create_mesh_box(depth + depth_base + finger_width, finger_width,
                           height)
    right = create_mesh_box(depth + depth_base + finger_width, finger_width,
                            height)
...
def find_scene_by_model_id(dataset_root, model_id_list):
    ...
    for scene_name in scene_names:
        try:
            scene_reader = xmlReader(
                os.path.join(dataset_root, 'scenes', scene_name, 'kinect',
                             'annotations', '0000.xml'))
        except:
            continue
        posevectors = scene_reader.getposevectorlist()
        for posevector in posevectors:
...
def generate_scene(scene_idx,
                   ...
                   camera='realsense'):
    camera_poses = np.load(
        os.path.join('scenes', 'scene_%04d' % (scene_idx, ), camera,
                     'camera_poses.npy'))
    camera_pose = camera_poses[anno_idx]
    if align:
        align_mat = np.load(
            os.path.join('camera_poses', '{}_alignment.npy'.format(camera)))
        camera_pose = align_mat.dot(camera_pose)
    camera_split = 'data' if camera == 'realsense' else 'data_kinect'
    # print('Scene {}, {}'.format(scene_idx, camera_split))
    scene_reader = xmlReader(
        os.path.join(scenedir % (scene_idx, camera), 'annotations',
                     '%04d.xml' % (anno_idx)))
    posevectors = scene_reader.getposevectorlist()
    obj_list = []
    mat_list = []
    ...
    for obj_idx, mat in zip(obj_list, mat_list):
        model = o3d.io.read_point_cloud(
            os.path.join(modeldir, '%03d' % obj_idx, 'nontextured.ply'))
        points = np.array(model.points)
...
def key_point_2_rotation(center_xyz, open_point_xyz, upper_point_xyz):
    ...
    **Output:**
    ...
    unit_open_point_vector = open_point_vector / np.linalg.norm(
        open_point_vector)
    unit_upper_point_vector = upper_point_vector / np.linalg.norm(
        upper_point_vector)
    rotation = np.hstack(
        (np.array([[0], [0], [1.0]]), unit_open_point_vector.reshape(
            (-1, 1)), unit_upper_point_vector.reshape((-1, 1))))
    return rotation
