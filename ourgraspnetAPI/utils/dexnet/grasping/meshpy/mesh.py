...
from autolab_core import (Direction, NormalCloud, Point, PointCloud,
                          RigidTransform)
...
class Mesh3D(object):
    ...
    def tri_normals(self, align_to_hull=False):
        ...
        if align_to_hull:
            ...
            ip = (tri_centers -
                  np.tile(hull_vertex, [tri_centers.shape[0], 1])).dot(n)
            if ip[0] > 0:
                normals = -normals
        return normals
    ...
    def remove_bad_tris(self):
        ...
        for t in self.triangles_:
            if (t[0] >= 0 and t[0] < num_v and t[1] >= 0 and t[1] < num_v
                    and t[2] >= 0 and t[2] < num_v):
                new_tris.append(t)
    ...
    def remove_unreferenced_vertices(self):
        ...
        for f in self.triangles_:
            new_triangles.append([
                reffed_v_new_ind[f[0]], reffed_v_new_ind[f[1]],
    ...
    def center_vertices_avg(self):
        ...
        This shifts the mesh without rotating it so that the centroid (mean) of
        all vertices is at the origin.
        """
    ...
    def center_vertices_bb(self):
        ...
        This shifts the mesh without rotating it so that the center of its
    ...
    def normalize_vertices(self):
        ...
        Transforms the vertices and normals of the mesh such that the origin of
        the resulting mesh's coordinate frame is at the center of the bounding
        box and the principal axes (as determined from PCA) are aligned with
        the vertical Z, Y, and X axes in that order.
        """
    ...
    def subdivide(self, min_tri_length=np.inf):
        ...
        while not tri_queue.empty():
            ...
            if (np.isinf(min_tri_length) and num_subdivisions_per_tri[j] == 0) or \
                ...
                for triplet in [[triangle[0], i_01, i_02],
                                ...
                                [triangle[2], i_02, i_12], [i_01, i_12, i_02]]:
        ...
        return Mesh3D(np.array(new_vertices),
    ...
    def transform(self, T):
        ...
        if self.normals_ is not None:
            return Mesh3D(vertices.copy(),
                          self.triangles.copy(),
                          normals=normals.copy(),
                          center_of_mass=com_tf.data)
        return Mesh3D(vertices.copy(),
                      self.triangles.copy(),
                      center_of_mass=com_tf.data)
    ...
    def update_tf(self, delta_T):
        ...
        new_T_obj_world = self.T_obj_world * delta_T.inverse().as_frames(
            'obj', 'obj')
        return Mesh3D(self.vertices,
                      self.triangles,
    ...
    def ray_intersections(self, ray, point, distance):
        """Returns a list containing the indices of the triangles that are
        intersected by the given ray emanating from the given point within some
        distance."""
        ...
        for i, tri in enumerate(self.triangles):
            if np.dot(ray, norms[i]) == 0.0:
                continue
            t = -1 * np.dot(
                (point - self.vertices[tri[0]]), norms[i]) / (np.dot(
                    ray, norms[i]))
    ...
    def get_T_surface_obj(self, T_obj_surface, delta=0.0):
        """Gets the transformation that puts the object resting exactly on the
        z=delta plane.
    ...
    def rescale_dimension(self, scale, scaling_type=ScalingTypeMin):
        """Rescales the vertex coordinates to scale using the given
        scaling_type.
    ...
    def convex_hull(self):
        ...
        if self.normals_ is None:
            cvh_mesh = Mesh3D(self.vertices_.copy(),
                              hull_tris.copy(),
                              center_of_mass=self.center_of_mass_)
        else:
            cvh_mesh = Mesh3D(self.vertices_.copy(),
                              hull_tris.copy(),
                              normals=self.normals_.copy(),
                              center_of_mass=self.center_of_mass_)
    ...
    def stable_poses(self, min_prob=0.0):
        ...
        prob_map = Mesh3D._compute_prob_map(
            list(self.face_dag_.nodes.values()), cvh_verts, cm)
    ...
    def resting_pose(self, T_obj_world, eps=1e-10):
        """Returns the stable pose that the mesh will rest on if it lands on an
        infinite planar worksurface quasi-statically in the given
        ...
        table_tri = np.array([[0, 0, 0], [1, 0, 0], [0, 1, 0]])
        ...
        for neighboring_tri in neighboring_tris:
            ...
            try:
                ...
                if np.all(alpha >= 0):
                    tri_normal = cvh_mesh._compute_basis(
                        [cvh_verts[i] for i in neighboring_tri])[2, :]
                    if tri_normal[2] < 0:
                    ...
                    lower = True
                    tri_center = np.mean(
                        [vertices_tf[i] for i in neighboring_tri], axis=0)
                    if topple_tri is not None:
                        topple_tri_center = np.mean(
                            [vertices_tf[i] for i in topple_tri], axis=0)
                        lower = (tri_normal.dot(topple_tri_center - tri_center)
                                 > 0)
                    if lower:
    ...
    def merge(self, other_mesh):
        ...
        combined_triangles[
            self.num_triangles:, :] = other_mesh.triangles + self.num_vertices
        ...
        return Mesh3D(combined_vertices, combined_triangles.astype(np.int32),
                      combined_normals)
    ...
    def find_contact(self, origin, direction):
        ...
        locations, _, tri_indices = self.trimesh.ray.intersects_location(
            [origin_obj.data], [direction_obj.data])
        ...
        normal_obj = Direction(
            self.trimesh.face_normals[tri_indices[closest_ind], :],
            frame='obj')
    ...
    def load(filename, cache_dir, preproc_script=None):
        ...
        if file_ext != Mesh3D.OBJ_EXT:
            obj_filename = os.path.join(
                cache_dir, file_root + Mesh3D.PROC_TAG + Mesh3D.OBJ_EXT)
        ...
        if not os.path.exists(obj_filename):
            raise ValueError(
                'Unable to open file %s. It may not exist or meshlab may not be installed.'
                % (filename))
    ...
    def _compute_com_uniform(self):
        """Computes the center of mass using a uniform mass distribution
        assumption.
    ...
    def _compute_proj_area(self, verts):
        """Projects vertices onto the unit sphere from the center of mass and
        ...
        a = math.acos(
            min(
                ...
                max(
                    -1,
                    np.dot(proj_verts[0], proj_verts[1]) /
                    (np.linalg.norm(proj_verts[0]) *
                     np.linalg.norm(proj_verts[1])))))
        b = math.acos(
            min(
                1,
                max(
                    -1,
                    np.dot(proj_verts[0], proj_verts[2]) /
                    (np.linalg.norm(proj_verts[0]) *
                     np.linalg.norm(proj_verts[2])))))
        c = math.acos(
            min(
                1,
                max(
                    -1,
                    np.dot(proj_verts[1], proj_verts[2]) /
                    (np.linalg.norm(proj_verts[1]) *
                     np.linalg.norm(proj_verts[2])))))
        s = (a + b + c) / 2
        ...
        try:
            return 4 * math.atan(
                math.sqrt(
                    math.tan(s / 2) * math.tan((s - a) / 2) * math.tan(
                        (s - b) / 2) * math.tan((s - c) / 2)))
        except:
            s = s + 0.001
            return 4 * math.atan(
                math.sqrt(
    ...
    def _compute_basis(self, face_verts):
        """Computes axes for a transformed basis relative to the plane in which
        input vertices lie.
        ...
        z_o = np.cross(face_verts[1] - face_verts[0],
    ...
    def _compute_face_dag(self):
        """Computes a directed acyclic graph (DAG) specifying the toppling
        structure of the mesh faces by:

        1) Computing the mesh convex hull     2) Creating maps from vertices
        and edges to the triangles that share them     3) Connecting each
        triangle in the convex hull to the face it will topple to, if landed on
        ...
        vertex_to_tri = {
        }  # mapping from vertex indidces to adjacent triangle lists
        ...
        for tri in cvh_tris:
            ...
            if not Mesh3D._point_in_tri(tri_verts, proj_cm):
                ...
                if len(closest_edges) == 1:
                    ...
                    closest_edge = Mesh3D._closer_segment(
                        proj_cm, closest_edges[0], closest_edges[1])
        ...
        self.face_dag_ = Mesh3D._FaceDAG(cvh_mesh, nodes, vertex_to_tri,
                                         edge_to_tri)
    ...
    class _Segment:
        ...
        def _contains_proj_point(self, point):
            ...
            """
            p1, p2 = self.p1, self.p2
            return (point[0] >= min(p1[0], p2[0])
                    and point[0] <= max(p1[0], p2[0])
                    and point[1] >= min(p1[1], p2[1])
                    and point[1] <= max(p1[1], p2[1])
                    and point[2] >= min(p1[2], p2[2])
                    and point[2] <= max(p1[2], p2[2]))
    ...
    class _FaceDAG:
        """A directed acyclic graph specifying the topppling dependency
        structure for faces of a given mesh geometry with a specific center of
        mass. Useful for quasi-static stable pose analysis.
    ...
    def _max_edge_length(tri, vertices):
        ...
        max_edge_len = max(
            ...
            max(np.linalg.norm(v1 - v0), np.linalg.norm(v2 - v1)))
        return max_edge_len

    @staticmethod
    def _proj_point_to_plane(tri_verts, point):
        """Project the given point onto the plane containing the three points
        in tri_verts.
    ...
    def _closest_segment(point, line_segments):
        """Returns the finite line segment(s) the least distance from the input
        point.
    ...
    def _closer_segment(point, s1, s2):
        """Compute which segment is closer to a given point by seeing which
        side of the midline between the two segments the point falls on.
