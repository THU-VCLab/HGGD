...
import numpy as np
from autolab_core import (NormalCloud, Point, PointCloud, RigidTransform,
                          SimilarityTransform)
...
class Sdf():
    ...
    def transform_to_world(self):
        """Returns an sdf object with center in the world frame of
        reference."""
    ...
    def is_out_of_bounds(self, coords):
        ...
        """
        return np.array(coords < 0).any() or np.array(
            coords >= self.dims_).any()
...
class Sdf3D(Sdf):
    ...
    def __init__(self,
                 ...
                 resolution,
                 use_abs=False,
                 T_sdf_world=RigidTransform(from_frame='sdf',
                                            to_frame='world')):
        self.data_ = sdf_data
        self.origin_ = origin
    ...
    def transform(self, delta_T):
        """Creates a new SDF with a given pose with respect to world
        coordinates.
        ...
        """
        new_T_sdf_world = self.T_sdf_world_ * delta_T.inverse().as_frames(
            'sdf', 'sdf')
        return Sdf3D(self.data_,
                     self.origin_,
                     self.resolution_,
                     use_abs=self.use_abs_,
    ...
    def _signed_distance(self, coords):
        ...
        if self.is_out_of_bounds(coords):
            ...
            closest_surface_coord = surface[np.argmin(
                np.linalg.norm(surface - coords, axis=-1))]
            ...
            if np.issubdtype(type(coords[0]), np.integer) and \
               ...
               np.issubdtype(type(coords[2]), np.integer):
                return self.data_[int(self.coords_buf_[0]),
                                  int(self.coords_buf_[1]),
                                  int(self.coords_buf_[2])]
    ...
    def gradient(self, coords):
        """Returns the SDF gradient at the given coordinates, interpolating if
        necessary.
        ...
        # regular indexing if integers
        if type(coords[0]) is int and type(coords[1]) is int and type(
                coords[2]) is int:
            self.coords_buf_ = self.coords_buf_.astype(np.int)
            return self.data_[self.coords_buf_[0], self.coords_buf_[1],
                              self.coords_buf_[2]]
    ...
    def surface_normal(self, coords, delta=1.5):
        """Returns the sdf surface normal at the given coordinates by computing
        the tangent plane using SDF interpolation.
        ...
        while dx <= delta:
            ...
            while dy <= delta:
                ...
                while dz <= delta:
                    ...
                    if np.abs(sdf_val) < self.surface_thresh_:
                        X.append([
                            index_coords[0], index_coords[1], index_coords[2],
                            sdf_val
                        ])
                    dz += delta
                dy += delta
    ...
    def _compute_surface_points(self):
        ...
        self.surface_vals_ = self.data_[self.surface_points_[:, 0],
                                        self.surface_points_[:, 1],
    ...
    def transform_dense(self, delta_T, detailed=False):
        ...
        if self.pts_ is None:
            ...
            self.pts_ = np.c_[x_ind.flatten().T,
                              np.c_[y_ind.flatten().T,
                                    z_ind.flatten().T]].astype(np.float32)
        ...
        if detailed:
            ...
            pts_tf_round[:, 0] = np.max(np.c_[np.zeros([num_pts, 1]),
                                              pts_tf_round[:, 0]],
                                        axis=1)
            pts_tf_round[:, 0] = np.min(
                np.c_[(self.dims_[0] - 1) * np.ones([num_pts, 1]),
                      pts_tf_round[:, 0]],
                axis=1)

            pts_tf_round[:, 1] = np.max(np.c_[np.zeros([num_pts, 1]),
                                              pts_tf_round[:, 1]],
                                        axis=1)
            pts_tf_round[:, 1] = np.min(
                np.c_[(self.dims_[1] - 1) * np.ones([num_pts, 1]),
                      pts_tf_round[:, 1]],
                axis=1)

            pts_tf_round[:, 2] = np.max(np.c_[np.zeros([num_pts, 1]),
                                              pts_tf_round[:, 2]],
                                        axis=1)
            pts_tf_round[:, 2] = np.min(
                np.c_[(self.dims_[2] - 1) * np.ones([num_pts, 1]),
                      pts_tf_round[:, 2]],
                axis=1)

            sdf_data_tf = self.data_[pts_tf_round[:, 0], pts_tf_round[:, 1],
                                     pts_tf_round[:, 2]]

        sdf_data_tf_grid = sdf_data_tf.reshape(self.dims_)
        tf_t = time.clock()

        logging.debug('Sdf3D: Time to transform coords: %f' %
                      (all_points_t - start_t))
        logging.debug('Sdf3D: Time to transform origin: %f' %
                      (origin_res_t - all_points_t))
        logging.debug('Sdf3D: Time to transfer sd: %f' % (tf_t - origin_res_t))
        return Sdf3D(sdf_data_tf_grid,
                     origin_tf,
                     resolution_tf,
                     use_abs=self._use_abs_,
                     T_sdf_world=self.T_sdf_world_)

    def transform_pt_obj_to_grid(self, x_sdf, direction=False):
        """Converts a point in sdf coords to the grid basis. If direction then
        don't translate.
    ...
    def transform_pt_grid_to_obj(self, x_grid, direction=False):
        """Converts a point in grid coords to the world basis. If direction
