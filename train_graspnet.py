    ...
    with torch.no_grad():
        for anchor_data, rgb, depth, grasppaths in tqdm(val_data,
            ...
            if epoch >= args.pre_epochs:
                ...
                #     points_all = get_scene_points(
                #         rgb.cuda(),
                #         depth.cuda(),
                #         perpoint_features,
                #         args.all_points_num,
                #         include_rgb=True).squeeze().cpu().numpy()
                #     cloud = points_all[:, :3]
                #     rgb = points_all[:, 3:6]
                #     pc = o3d.geometry.PointCloud()
                #     pc.points = o3d.utility.Vector3dVector(cloud)
                #     pc.colors = o3d.utility.Vector3dVector(rgb)
                #     o3d.visualization.draw_geometries([pc] + grasp_geo)

                # dump grasp for graspnet
                # dump_grasp(epoch, batch_idx, pred_gg,
                #            val_data.dataset.scene_list())
