...
from util import (batch_center_depth, batch_framexy_depth_2_xyz,
                  batch_rgbdxyz_2_rgbxy_depth, get_batch_key_points,
                  mask_grasp_group, remove_invisible_grasp_points,
                  to_grasp_group, to_open3d_geometry_list, to_opencv_image,
                  to_rect_grasp_group)

from ourgraspnetAPI import GraspNet

graspnet_root = '/ssd/graspnet'  # ROOT PATH FOR GRASPNET
output_path = '/data/6dto2drefine_test'
camera = 'realsense'
dataset = GraspNet(graspnet_root, camera=camera, split='train')
# dataset.showSceneGrasp(sceneId=124, annId=3, camera='kinect', numGrasp=100, coef_fric_thresh=1.0)
...
def run(sceneIds):
    ...
    for scene in sceneIds:
        ...
        for ann in annIds:
            ...
            # local_cloud_ = o3d.geometry.PointCloud()
            # local_cloud_.points = o3d.utility.Vector3dVector(local_clouds[0][:, :3])
            # frame = o3d.geometry.TriangleMesh.create_coordinate_frame(0.1)
            # o3d.visualization.draw_geometries([local_cloud_, frame])
